...
variable "database_url_parameter" {
  description = "SSM parameter holding the Supabase transaction-mode URL for staging."
  type        = string
  default     = "/monevo/staging/database-url"
}
...
variable "gateway_secret_parameter" {
  ...
  type        = string
  default     = "/monevo/staging/gateway-secret"
}
